...
SEGMENT_LABELS = {
    "P1": "Busy Professional",
    "P2": "Health-Conscious Family Buyer",
    "P3": "Budget & Value Seeker",
    "P4": "Performance Nutrition Buyer",
    "P5": "Practical Value Buyer",
}
PERSONA_NAMES = {
    "P1": "Marcus",
    "P2": "Jenna",
...
def run(session_id: str) -> None:
    ...
    # TITLE
    # ═══════════════════════════════════════════════════════════════════════════
    h1("GEO Audit: Which QSR Brands Win ChatGPT Recommendations — and Why")
    line()
    ...
    line()
    line(
        "We built five fictional but research-grounded consumer profiles representing "
        "real buyer segments in the QSR market: the **Busy Professional**, "
        "the **Health-Conscious Family Buyer**, the **Budget & Value Seeker**, "
        "the **Performance Nutrition Buyer**, and the **Practical Value Buyer**. "
        "Each profile included a detailed character description — job, lifestyle, "
        "priorities, price sensitivity — injected as context before every ChatGPT question."
    )
    line()
    line(
        "We then asked 25 questions per segment covering five intent types: "
        "spontaneous cravings, dietary or budget constraints, comparative decisions, "
        "discovery and trends, and meal planning. Each question was asked three times "
        "to measure consistency. Every brand mention in every response was extracted "
    ...
    line(
        ...
        "So \"quick lunch,\" \"fast meal,\" and \"grab something on the go\" would all produce "
    ...
    line(
        ...
        "\"is this about speed?\", \"is this about health?\", \"is this about family?\" "
    ...
    line(
        ...
        "A score of 65 means Chipotle's website points in a very similar direction to "
        "what ChatGPT says when answering budget-shopping questions — the vocabulary, "
        "concepts, and framing overlap strongly. "
        "A score of 42 for the same pair means Chipotle's site is talking about things "
        "(brand story, sustainability, ingredient sourcing) that don't map onto the vocabulary "
        "ChatGPT reaches for when a price-conscious person asks for fast food advice."
    )
    line()
    line(
    ...
    for pid in personas:
        ...
        # Narrative
        narrative = (
            f"When someone in the **{seg}** segment asks ChatGPT for fast food advice, "
    ...
    line(
        "- **Editorial mentions:** Earn coverage from food media, local press, and "
        "review sites that are likely in AI training corpora (Eater, Serious Eats, "
        "local newspaper food sections, Reddit food communities)."
    )
    line()
    ...
    line()
    line(
        "The five segments were designed as research archetypes grounded in QSR consumer "
